...
// MongoDB Connection
mongoose
  .connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/coopmap')
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      type LeanQuery = ReturnType<typeof Company.find>;
      let query: LeanQuery = Company.find(filter)
        .skip(skip)
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const { q, limit = '20' } = req.query as Record<string, string | undefined>;
      ...
      const companies = await Company.find({
        ...
        .limit(parseInt(limit, 10))
        .select(
          'รายชื่อสถานประกอบการ สำนักวิชา หลักสูตร จังหวัดที่ตั้ง latitude longitude',
        )
        .lean();
...
app.get(
  ...
  async (_req: Request, res: Response) => {
    try {
      const [faculties, programs, provinces, businessTypes, companySizes] =
        await Promise.all([
          Company.distinct('สำนักวิชา', { isActive: true }),
          Company.distinct('หลักสูตร', { isActive: true }),
          Company.distinct('จังหวัดที่ตั้ง', { isActive: true }),
          Company.distinct('businessType', {
            isActive: true,
      ...
      res.json({
        success: true,
        data: {
          faculties: (faculties as string[]).sort(),
          programs: (programs as string[]).sort(),
          provinces: (provinces as string[]).sort(),
          businessTypes: (businessTypes as string[]).sort(),
          companySizes: (companySizes as string[]).sort(),
        },
      });
    } catch (error) {
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const { lat, lng, radius = '50', limit = '20' } = req.query as Record<
        string,
        string | undefined
      >;

...
const careerFieldKeywords = [
  { field: 'Tech & Data', keywords: ['เทคโนโลยี', 'ไอที', 'ซอฟต์แวร์', 'คอมพิวเตอร์', 'ดิจิทัล', 'โปรแกรม', 'ข้อมูล', 'tech', 'it ', 'software', 'digital', 'data', 'ai', 'cyber', 'network', 'เน็ตเวิร์ก', 'ระบบ', 'แอป', 'เว็บ', 'โดรน'] },
  { field: 'Engineering', keywords: ['วิศวกร', 'ก่อสร้าง', 'โยธา', 'เครื่องกล', 'ไฟฟ้า', 'อิเล็กทรอนิกส์', 'โรงงาน', 'ผลิต', 'engineer', 'construction', 'industrial', 'manufacturing', 'ยานยนต์'] },
...
function getCareerFields(name) {
  if (!name) return ['General'];
  const lower = name.toLowerCase();
  const matched = careerFieldKeywords
...
function getRegion(province) {
  return provinceToRegion[province?.trim()] ?? 'Unknown';
}

function getCoords(province) {
  const base = provinceCoordinates[province?.trim()] ?? { lat: 13.7563, lng: 100.5018 };
  const offset = () => (Math.random() - 0.5) * 0.08;
  return { lat: +(base.lat + offset()).toFixed(6), lng: +(base.lng + offset()).toFixed(6) };
}

// Read CSV
const csvText = readFileSync(
  new URL('../รายชื่อสถานประกอบการทั้งหมด 19 Feb 2026.csv', import.meta.url),
  'utf-8'
);

const result = parse(csvText, { header: true, skipEmptyLines: true });

const data = result.data.map((row, idx) => {
  const province = row['PROVINCENAME']?.trim() ?? '';
  const coords = getCoords(province);
  return {
    id: row['NO.'] ?? row['NO'] ?? String(idx + 1),
    ...
    careerFields: getCareerFields(row['COMPANYNAME']),
    lat: coords.lat,
    lng: coords.lng,
  };
});
...
console.log(`✅ Generated companies.json with ${data.length} companies`);
